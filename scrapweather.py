...
def read_one_month_from_gc(station, dt):
    ...
    with open(tmp_address) as weather_file:
        ...
        for row in reader:
            ...
            weather_data_hourly_list.append(weather_data_dict)
    remove(tmp_address)
...
def read_multiple_month_from_gc(station, dt_start, dt_end=None):
    ...
    for dt in rrule.rrule(rrule.MONTHLY, dtstart=dt_start, until=dt_end):
        weather_data_hourly_list.append(read_one_month_from_gc(station, dt))

    return weather_data_hourly_list

x = read_multiple_month_from_gc('10999', datetime(year=1994, month=1, day=1))
for a in x:
    print(a)
